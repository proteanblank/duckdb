...
struct DuckDBPyConnection {
	...
	static unique_ptr<DuckDBPyConnection> connect(string database, bool read_only, bool enable_copy) {
		auto res = make_unique<DuckDBPyConnection>();
		DBConfig config;
		if (read_only) {
...
static DuckDBPyConnection *default_connection() {
	if (!default_connection_) {
		default_connection_ = DuckDBPyConnection::connect(":memory:", false, true);
	}
...
PYBIND11_MODULE(duckdb, m) {
	m.def("connect", &DuckDBPyConnection::connect,
	      ...
	      py::arg("database") = ":memory:", py::arg("read_only") = false, py::arg("enable_copy") = true);
